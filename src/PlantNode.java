...
public class PlantNode {
    ...
    public PlantNode(int x, int y, double direction, Color color, double branchProb, double maxBranchProb, double leafProb, int height) {
    ...
    private Plant createBranch(double branchProb, double maxBranchProb, int height) {
        if(Math.random() < branchProb){
            ...
            return new Plant(coord.getX(), coord.getY(), branchDir, 1, 4, 30, 10, height/5, height/2, 30, true, 0, maxBranchProb - 0.01, 0.01, color, false);
        }
        return null;
    }
